...
class RecipeTest(APITestCase):
    ...
    def test_patch_recipe_detail(self):
        for recipe in self.recipes:
            url = recipe.get_absolute_url(category='recipe_detail')
            response = self.client.patch(path=url,HTTP_AUTHORIZATION=f"Bearer {self.access_token}",
...
class ReviewTest(APITestCase):
    ...
    def test_post_review(self):
        for recipe in self.recipes:
            ...
            response = self.client.post(path=url,HTTP_AUTHORIZATION=f"Bearer {self.access_token}",
